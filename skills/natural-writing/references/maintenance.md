...
10. **aboudjem/humanizer-skill**, https://github.com/aboudjem/humanizer-skill, MIT, Copyright (c) 2026 Adam Boudjemaa. 53 numbered patterns, most of them tracing to the same Wikipedia catalog we already read directly, so read it for its gates rather than its rules. Two things to leave alone. Its Concretizer pass instructs turning "improves performance" into an invented latency figure, which our no-fabrication rule forbids outright. And its 0-100 score ships in two contradictory formulas, self-declared as untrained heuristics, against a stance we hold on purpose: report patterns, never score authorship.
